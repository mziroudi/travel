...
const MediaItem = ({ item, className, onClick }: { item: MediaItemType, className?: string, onClick?: () => void }) => {
    ...
    return (
        <div className={`${className} relative overflow-hidden group`}>
            ...
            {error ? (
                <div className="absolute inset-0 flex items-center justify-center bg-zinc-100">
                    <div className="text-zinc-500 text-sm">Failed to load image</div>
                </div>
            ) : (
                <img
                    src={item.url}
            ...
            {item.attribution && !isLoading && !error && (
                <a
                    href={item.attribution.link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="absolute bottom-0 right-0 p-2 text-[10px] text-white/70 hover:text-white/90 bg-black/50 rounded-tl-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200"
                    onClick={(e) => e.stopPropagation()}
                >
                    Photo by {item.attribution.name}
                </a>
            )}
        </div>
    );
